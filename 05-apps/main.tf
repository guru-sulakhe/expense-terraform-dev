...
module "ansible" {
  ...
  subnet_id              = local.public_subnet_id # selecting one public subnet from the list
  ami = data.aws_ami.ami_info.id
  user_data = file("expense.sh") # reading the data from expense.sh and running the data during the instance creation  
  ...
  depends_on = [ module.backend, module.frontend ] #explicitly depends on backend and frontend modules 
